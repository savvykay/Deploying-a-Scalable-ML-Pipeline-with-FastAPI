...
project_path = os.path.dirname(os.path.abspath(__file__))  # gets the current script directory, more usable in other systems than an absolute path
...
# Process the training data using the process_data function in training mode
...
# Process the test data using the process_data function in inference mode
...
for col in cat_features:
    for slice_value in sorted(test[col].unique()):
        ...
        with open("slice_output.txt", "a") as f:
            ...
            print(f"Precision: {p_slice:.4f} | Recall: {r_slice:.4f} | F1: {fb_slice:.4f}", file=f)